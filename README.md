...
6. **리뷰 판정** (Jev, 식당당 1요청) — 최근 리뷰 12개로 한 번에 묻습니다.
   ...
   - 체인점: 가게 이름으로 Jev가 판정(전국 대형 브랜드만 잘 앎) + 자주 보이는 외식 프랜차이즈 이름 목록. 🏷 표시, "숨은 맛집"에서는 뺍니다.
   - 📈 최근 리뷰 급증: 최근 리뷰 30개의 작성일로 속도를 재서, 지금 속도로 1년이면 지금까지 전체 리뷰의 3배 이상이면 표시만 합니다(새로 연 가게·연휴 몰림도 걸리므로 거르지는 않음).
7. **업종 궁합** (Jev) — 조건이 있으면 업종 이름만 보고 안 맞는 업종(예: 아이랑 → 술집)을 뺍니다.
8. **내 근처·끼니** — `--near 위도,경도`(웹 📍내 근처)면 지역 없이 그 주변 1.5km, 걷는 시간(직선×1.3÷분당 70m)을 붙입니다. `--meal 점심`이면 오늘 쉬는 곳과 네이버 영업 안내 한 줄("16:00에 영업 시작")로 봐서 그 끼니에 닫는 곳을 뺍니다(요일별 영업시간은 목록에 없어 대략적).
9. **👍/👎·정답지** — 웹 카드의 👍/👎는 `~/.config/naver-matjip/feedback.json`에 저장되고, 👎한 곳은 다음부터 빠지며 👍한 곳은 정답지(`~/.config/naver-matjip/gold.json`)에 쌓입니다. `python3 eval/run_rank_eval.py`가 정답지 가게가 그룹 상위 3곳(보임)·통과에 드는 비율을 잽니다. 개인 취향 데이터라 레포에 넣지 않습니다.
10. **웹 검색창** (Jev) — 규칙이 조건으로 남긴 낱말을 Jev가 메뉴/조건/군더더기로 다시 가릅니다(예: "문정 점심 맛집"의 "점심"은 뺌).
